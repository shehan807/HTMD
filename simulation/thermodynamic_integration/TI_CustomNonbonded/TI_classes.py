...
def define_energy_function_full( lambda_rep_string , lambda_attrac_string ):
    ...
    string='{}*A/r^12 - {}*C/r^6 ; A=sqrt(A1*A2) ; C=sqrt(C1*C2) ; A1=4*eps1*sig1^12 ; A2=4*eps2*sig2^12 ; C1=4*eps1*sig1^6 ; C2=4*eps2*sig2^6 '.format(lambda_rep_string, lambda_attrac_string)
    return string
...
# add small to C so that we don't divide by zero here in combination rule for sigma ...
def define_energy_function_rep( lambda_rep_string , lambda_attrac_string ):
    # lambda_rep scales repulsive part, lambda_attrac scales attractive part
    #string='{0}*A/rb^12 + {1}; rb=(0.5*sigboth^6*(1.0-{0})^2+r^6)^(1.0/6.0) ; sigboth=(A/C)^(1/6) ;  A=sqrt(A1*A2) ; C=(sqrt(C1*C2)+1E-8) ; A1=4*eps1*sig1^12 ; A2=4*eps2*sig2^12 ; C1=4*eps1*sig1^6 ; C2=4*eps2*sig2^6 '.format(lambda_rep_string, lambda_attrac_string)
...
class TI(object):
    def __init__(self, solutename, soluteID, atomshells, system, modeller, forcefield, integrator, platform, properties, use_SCF_lambda_pol, interaction_type, NPT_simulation):
          ...
          print("printing alchemical_atomset, all_atomset, and nonalchemical_atomset")
          ...
          # only create alchemical if not electrostatic
          ...
          # only exclusions if non-electrostatic ...
